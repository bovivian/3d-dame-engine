...
#define CAPACITYOFINVENTORY 20
#define JUMPHIGHT 5.0f
...
void Player::Init(SkinnedModel * model, Physics * physics, AnimationPack animPack)
{
	...
	// Inventory
	inventory.Init(CAPACITYOFINVENTORY);
	...
	if(animPack.runAnimation != NULL)
		animPack.runAnimation->SetAnimationSpeed(0.002f + (0.0001f * runSpeed));
}
...
void Player::Update(VulkanInterface * vulkan, Camera * camera)
{
	...
	if (inputEnabled)
	{
		...
		if (cameraYaw < 0.0f) cameraYaw = 359.9999f;
		if (cameraYaw > 360.0f) cameraYaw = 0.1f;
		if (cameraPitch > 50.0f) cameraPitch = 49.9999f;
		if (cameraPitch < -4.0f) cameraPitch = -3.9999f;
	...
	{
		camera->SetOrbitParameters(GetPosition(), glm::vec3(0.0f, 180.0f, 0.0f), 7.0f);
...
{	
	...
	{
		sndPlaySound("data/sounds/jump.wav", SND_ASYNC);
		btVector3 velocity = playerBody->getLinearVelocity();
		playerBody->setLinearVelocity(btVector3(0.3f * velocity.x(), JUMPHIGHT, velocity.z()));
		isJumping = true;
